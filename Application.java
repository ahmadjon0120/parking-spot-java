...
public class Application implements ActionListener {
    ...
    public Application(CarPark carpar) {
        this.carpar = carpar;
        prepareApplication();
    }
    ...
    /**
     *
     * Preparing the Application by making a new JFrame first, then adding 3 panels
     * namely, imagePanel, headerPanel and main to hold the
    ...
    private void prepareApplication() {
        ...
        JPanel imagePanel = new JPanel(new GridLayout(2, 1)); // adding header panel containing image and text with 2
                                                              // rows 1 column
        JLabel textLabel = new JLabel("NEW SWINBURNE CAR PARK SYSTEM"); // Text label for headerpanel
    ...
    public void addVisStafParking() {
        ...
        boolean query = false;
        while (!query) {
            String vis = JOptionPane.showInputDialog("Enter Number of Visitor Parking");
            String stf = JOptionPane.showInputDialog("Enter Number of Staff Parking");
            if (vis != null && stf != null) {

                if (!vis.chars().allMatch(Character::isDigit) || !stf.chars().allMatch(Character::isDigit)) {
                    JOptionPane.showMessageDialog(null, "Please enter a numeric value eg. (1 or 5 )");

                } else if (stf.equals("") || vis.equals("")) {
                    JOptionPane.showMessageDialog(null, "Please don't leave input blank. Enter a numeric value eg. (1 or 5 )");
     ...
     * This function is implemented in an anonymous listener in the
     * prepareApplication.
     *
     */
    ...
    public void parkCar() {
        String rego = JOptionPane.showInputDialog("The Rego Number of the Car e.g. (A1234)");
        String value = "";
        if (rego != null) {
            if (rego.length() == 5 && !carpar.findVehicleRego(rego).contains(rego)
                ...
                if (carpar.getAvailableParkingSlots().isEmpty()) {
                    ...
                    headerpanel.removeAll();
                    showAllParkings();
                    String parkingSpot = JOptionPane.showInputDialog("Enter Spot You Want to Park e.g.: (S001)");
                    String staffMember = JOptionPane.showInputDialog("Are you a staff member or visitor? (Please enter <staff> or <visitor>)");
                ...
                JOptionPane.showMessageDialog(headerLabel, "Make Sure Registration follows proper format e.g.(A1234).",
    ...
    /**
     * Main function to run the Application application. A carpark instance is added
     * to a Application instance and showHomeScreen is called.
     */

    public static void main(String[] args) {
        CarPark carpar = new CarPark();
        Application newApplication = new Application(carpar);
        newApplication.showHomeScreen();
    }
}
